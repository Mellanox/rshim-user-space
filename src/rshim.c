...
#define RSHIM_NET_POLL_IDLE_INTERVAL 10
...
static void rshim_timer_run(void)
{
  ...
  for (i = 0; i < RSHIM_MAX_DEV; i++) {
    ...
    if (bd) {
      ...
      if (bd->net_fd >= 0) {
        if (rshim_timer_ticks % bd->net_poll_interval == 0) {
          tx_activity = rshim_net_tx(bd);
          rx_activity = rshim_net_rx(bd);

          /* Adjust polling interval based on activity */
          if (tx_activity || rx_activity)
            bd->net_poll_interval = RSHIM_TIMER_INTERVAL;
          else
            bd->net_poll_interval = RSHIM_NET_POLL_IDLE_INTERVAL;
        }
      }
    }
  }
